defmodule Websockets do
  ...
  def start(room_url_key) do
    ...
    loop(socket, Timex.now, 0)
  end

  defp loop(socket, _ping_time, time_elapsed_since_last_ping) when time_elapsed_since_last_ping >= 60 do
    socket |> Socket.Web.send!({:text, "PING\tshowroom"})
  ...
  defp loop(socket, ping_time, _time_elapsed_since_last_ping) do
    {:text, message} = socket |> Socket.Web.recv!
    Logger.info(message)

    loop(socket, ping_time, Timex.diff(Timex.now, ping_time, :seconds))
  end
